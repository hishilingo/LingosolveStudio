...
namespace LingosolveStudio.Utilities
{
    ...
    public class ImageHelper
    {
        ...
        /// Finds the 1st and 99th percentile luminance values and
        /// linearly maps them to the full 0-255 range.
        /// </summary>
        public static Bitmap AutoContrast(Bitmap source)
        {
            Bitmap src = source.PixelFormat == PixelFormat.Format32bppArgb
                ? source
                : ConvertTo32bpp(source);
            ...
            {
                int lum = (int)(0.299 * buffer[i + 2] + 0.587 * buffer[i + 1] + 0.114 * buffer[i]);
            ...
            for (int i = 0; i < 256; i++)
            {
                ...
                if (cumulative >= lowThreshold) { low = i; break; }
            }
            cumulative = 0;
            for (int i = 255; i >= 0; i--)
            {
                cumulative += histogram[i];
                if (cumulative >= (totalPixels - highThreshold)) { high = i; break; }
            }
            ...
            for (int i = 0; i < 256; i++)
            {
                lut[i] = (byte)Math.Min(255, Math.Max(0, (int)((i - low) * scale)));
            }
            ...
            for (int i = 0; i < bytes; i += 4)
            {
                result[i]     = lut[buffer[i]];     // B
                result[i + 1] = lut[buffer[i + 1]]; // G
                result[i + 2] = lut[buffer[i + 2]]; // R
                result[i + 3] = buffer[i + 3];       // A
            }
            ...
            Marshal.Copy(result, 0, outData.Scan0, result.Length);
